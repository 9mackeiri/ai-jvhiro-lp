...
3. 週 1 回、その週に投稿した回のインサイトを 1 本につき 3 枚ずつスクショし（概要の一番上／閲覧数の推移とスキップ率／再生時間のグラフと上位ソース。それより前の回の 1 枚目は任意）、iCloud `Cursor/インスタ投稿/インサイト/<その日の日付>/` に入れ、**新しいチャットで `/insight`** と打つ（sns-marketer の分析 → レポート保存 → コミットまで進む。重い調査・分析はここに寄せる）
