...
- 各セルの中央に大きなロゴ（高さ 400px＝セル高の約 47%）、その下に太字の名前、必要なら小さく一言
...
| note | `logos/note_logo.svg`（無加工） | noteヘルプセンター「ブランドガイドライン（ロゴデータ・カラー）」 https://www.help-note.com/hc/ja/articles/360000235582 の `note Visual Identity.zip` 内 `main/logo.svg`。同梱のガイドライン PDF を `logos/note_Visual_Identity.pdf` として保存 |
...
- note：基本色は黒（#000000）、サブカラーは白。影・装飾・変形・アウトライン化は不可。周囲に n と o の間隔の 1.7 倍の余白。ヘルプページには「ソーシャルアイコンと並列に並ぶ場合は square（角丸の n）を使う」とあるが、今回は指示どおりワードマーク（logo.svg）を使用している。差し替える場合は `note Visual Identity/main/square.svg`
